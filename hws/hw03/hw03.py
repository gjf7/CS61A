...
def missing_digits(n):
    ...
    if n < 10:
        return 0
    high, low = n % 10, n // 10 % 10
    return missing_digits(n // 10) + (high - low - 1 if high > low + 1 else 0)
...
def count_change(total):
    ...
    def helper(total, small_coin):
        if total == 0:
            ...
            with_small = helper(total - small_coin, small_coin)
            no_small = helper(total,small_coin + 1 if small_coin == 1 else small_coin * 2)
