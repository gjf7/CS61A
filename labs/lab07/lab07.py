...
def make_glookup(class_assignments):
    ...
    def fn(assignment, points):
        nonlocal currentTotalPoints, currentEarnedPoints
        currentTotalPoints += class_assignments[assignment]
        currentEarnedPoints += points
        return currentEarnedPoints / currentTotalPoints
    return fn
...
def make_advanced_counter_maker():
    ...
    def make_counter():
        counter = 0
        def someone_counter(message):
            nonlocal counter, global_counter
            if message == "count":
                counter += 1
                return counter
