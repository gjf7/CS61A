...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    selectedPara = list(filter(select, paragraphs))
    if len(selectedPara) <= k:
        return ""
    else:
        return selectedPara[k]
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    "*** YOUR CODE HERE ***"
    correct_count = 0
    for index, ref_word in enumerate(reference_words):
        if index < typed_words_len and typed_words[index] == ref_word:
            correct_count += 1
...
def report_progress(typed, prompt, id, send):
    ...
    for index,word in enumerate(typed):
        if prompt[index] == word:
            ...
            break
    progress = correct_counter / len(prompt)
...
def time_per_word(times_per_player, words):
    ...
    time_stamp_length = len(times_per_player[0])
    for i in range(player_len):
        current = []
        for j in range(1, time_stamp_length):
            current.append(times_per_player[i][j] - times_per_player[i][j - 1])
...
def fastest_words(game):
    ...
    for i in words:
        # find player who has fastest speed per word
        timeMin = float('inf') # Python's max numbers
        playerIndex = -1
        word = word_at(game, i)
        for j in players:
            currentPlayerTime = time(game, j, i) 
            if currentPlayerTime < timeMin:
                timeMin = currentPlayerTime
                playerIndex = j
        ret[playerIndex].append(word)
    return ret
